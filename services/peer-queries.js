...
const ORG1_CLIENT_CONNECTION_PROFILE_PATH = './profiles/org1-client.yaml'
const ORG2_CLIENT_CONNECTION_PROFILE_PATH = './profiles/org2-client.yaml'
...
exports.getPeerInfo = async function(org,peerName) {
    try {
        ...
        let network = []
        if (peerQueryResponse.local_peers.Org1MSP && peerQueryResponse.local_peers.Org1MSP.peers.length > 0){
            console.log(`\tOrg1MSP: ${peerQueryResponse.local_peers.Org1MSP.peers[0].endpoint}`)
            network.push(peerQueryResponse.local_peers.Org1MSP.peers[0].endpoint)
        }
        if (peerQueryResponse.local_peers.Org2MSP && peerQueryResponse.local_peers.Org2MSP.peers.length > 0){
            console.log(`\tOrg2MSP: ${peerQueryResponse.local_peers.Org2MSP.peers[0].endpoint}`)
            network.push(peerQueryResponse.local_peers.Org2MSP.peers[0].endpoint)
        }
...
async function setupClient(org) {
    ...
    if (org == 'org1') {
        client.loadFromConfig(ORG1_CLIENT_CONNECTION_PROFILE_PATH)
    } else if (org == 'org2') {
        client.loadFromConfig(ORG2_CLIENT_CONNECTION_PROFILE_PATH)
    } else {
