...
exports.joinChannel = async function(orgName,channelName,peers){
    ...
    try {
        ...
        for (let index = 0; index < peers.length; index++) {
            ...
            const tls_cacert = `../crypto-config/peerOrganizations/${orgName}.example.com/peers/peer${index}.${orgName}.example.com/tls/ca.crt`;
            const data = fs.readFileSync(path.join(__dirname, tls_cacert));
            const peer = client.newPeer(peers[index], {
              'pem': Buffer.from(data).toString(),
              'ssl-target-name-override': `peer${index}.${orgName}.example.com`
            });
            targets.push(peer);
        }
        let join_request = {
...
exports.upgradeChaincode = async function(orgName,peers,chaincodeId,chaincodeVersion,channelNames,chaincodeType,args){
    //setup the channel
    let client = await setupClient(orgName);
    let channel = await setupChannel(client,channelNames);
    let error_message = null;
